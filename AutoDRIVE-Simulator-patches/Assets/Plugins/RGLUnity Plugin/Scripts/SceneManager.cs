    ...
    public class SceneManager : MonoBehaviour
    {
        ...
        private static IEnumerable<RGLObject> IntoRGLObjectsUsingCollider(IEnumerable<GameObject> gameObjects)
        {
            foreach (var gameObject in gameObjects)
            {
                if (gameObject.TryGetComponent<Collider>(out var collider))
                {
                    ...
                    var rglObject = ColliderToRGLObject(collider);
                    if (rglObject == null)
                    {
                        continue;
                    }
        ...
        private static IEnumerable<RGLObject> IntoRGLObjectsHybrid(IEnumerable<GameObject> gameObjects)
        {
            ...
            foreach (var renderer in GetUniqueRenderersInGameObjects(gameObjects))
            {
                ...
                {
                    var rglObject = MeshRendererToRGLObjectWithColliderFallback(mr);
                    if (rglObject == null)
                    {
                        continue;
                    }
        ...
        private static RGLObject ColliderToRGLObject(Collider collider)
        {
            ...
            if (mesh == null)
            {
                Debug.LogWarning($"Shared mesh of {collider.gameObject} is null, skipping collider");
                return null;
            }
            ...
            if (!sharedMeshes.ContainsKey(meshId))
            {
                ...
                {
                    Debug.LogWarning($"Could not upload collider mesh of {collider.gameObject} to RGL, skipping collider. Reason: {ex.Message}");
                    return null;
                }
            }
        ...
        private static RGLObject MeshRendererToRGLObjectWithColliderFallback(MeshRenderer meshRenderer)
        {
            ...
            {
                var meshRglObject = MeshFilterToRGLObject(meshFilter);
                if (meshRglObject != null)
                {
                    return meshRglObject;
                }
            }

            if (gameObject.TryGetComponent<Collider>(out var collider) && collider.enabled)
            {
                return ColliderToRGLObject(collider);
            }
        ...
        private static RGLObject MeshFilterToRGLObject(MeshFilter meshFilter)
        {
            ...
            if (mesh == null)
            {
                Debug.LogWarning($"Shared mesh of {meshFilter.gameObject} is null, skipping");
                return null;
            }
            string meshId = $"r#{mesh.GetInstanceID()}";
            if (!sharedMeshes.ContainsKey(meshId))
            {
                ...
                {
                    Debug.LogWarning($"Could not upload mesh of {meshFilter.gameObject} to RGL, skipping mesh. Reason: {ex.Message}");
                    return null;
                }
            }
