...
public class DataRecorder : MonoBehaviour
{
    ...
    */
    ...
    public bool checkSaveLocation()
    {
        ...
        {
            SetFileBrowserMode(true);

            bool opened = SimpleFileBrowser.ShowSaveDialog(
                OpenFolder,
    ...
    private void SetFileBrowserMode(bool enabled)
    {
        // SimpleFileBrowser가 별도 EventSystem을 생성하므로 기본 EventSystem은 잠시 비활성화한다.
        // 대신 커서는 항상 보이도록 강제해서 Linux/X11 환경에서 커서가 사라진 것처럼 보이는
        // 상태를 방지한다.
        if (eventSystem != null)
            eventSystem.SetActive(!enabled);
