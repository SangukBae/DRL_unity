...
from tqc_ieqn_agent import Agent
from environment_interface import EnvInterface
from file_manager import DirectoryManager, load_yaml


class TrainTQC_IEQN(EnvInterface):
    def __init__(self):
        """Initialize TQC_IEQN training node"""
        super().__init__("train_tqc_ieqn_node")
        ...
        # ----------------------------
        train_cfg_path = self._find_config_file("train_tqc_ieqn_config.yaml", user_param_path)
        if not train_cfg_path:
            self.get_logger().error("Could not find 'train_tqc_ieqn_config.yaml'")
            raise FileNotFoundError("train_tqc_ieqn_config.yaml not found")
        ...
        # ----------------------------
        hparams_path = self._find_config_file("hyperparameters_tqc_ieqn.yaml", user_param_path)
        if not hparams_path:
            self.get_logger().error("Could not find 'hyperparameters_tqc_ieqn.yaml'")
            raise FileNotFoundError("hyperparameters_tqc_ieqn.yaml not found")
    ...
    def log_training_setting_data(self):
        ...
        self.get_logger().info("TQC_IEQN Training Configuration")
        self.get_logger().info("=" * 50)
