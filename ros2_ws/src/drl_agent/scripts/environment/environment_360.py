...
class Environment(Node):
    ...
    def __init__(self):
        ...
        self.fov_min = -math.pi
        self.fov_max = math.pi
        self.bin_width = (self.fov_max - self.fov_min) / self.environment_dim
        self._inv_bin_width = 1.0 / self.bin_width
        ...
        for i in range(self.environment_dim):
            a0 = start + i * width
            a1 = a0 + width
            self.bins.append([a0, a1])
        # 마지막 경계 살짝 확장해 경계각 포함(부동소수점 안전)
        self.bins[-1][1] = np.pi + 1e-6
    ...
    def update_environment_state(self, scan: LaserScan):
        """
        LaserScan -> 정면 정렬(원형 roll) -> 균등 섹터 binning(min-pool) -> 상태 갱신
        - 상태 값은 '미터' 단위 그대로 유지(self.lidar_max_range로 클립)
        - 필요시 0~1 정규화는 마지막에 주석 해제
        """
        try:
            r_raw = np.asarray(scan.ranges, dtype=np.float32)
            N = r_raw.size
            if N == 0:
                return

            # ── 유효 레이 존재 여부 확인(모두 inf/NaN이면 상태 유지) ─────────────────
            finite_mask = np.isfinite(r_raw)
            if not finite_mask.any():
                return  # 이전 상태 유지

            # ── 파라미터/상수 ───────────────────────────────────────────────────────
            rmin = float(scan.range_min)
            rmax = min(float(scan.range_max), float(self.lidar_max_range))
            a_min = float(scan.angle_min)
            a_inc = float(scan.angle_increment)

            # 선택적 yaw 오프셋(센서 프레임 정면과 base_link 정면 사이 각도 차)
            # __init__에 self.scan_yaw_offset를 정의하지 않았다면 0.0으로 간주
            yaw_off = getattr(self, "scan_yaw_offset", 0.0)  # [rad], 기본 0.0

            # ── 거리 벡터 전처리: inf/NaN -> rmax, [rmin, rmax]로 클립 ───────────────
            r = r_raw.copy()
            r[~np.isfinite(r)] = rmax
            np.clip(r, rmin, rmax, out=r)

            # ── 각도 증가가 음수면 뒤집어서 CCW 기준으로 정규화 ────────────────────
            if a_inc < 0.0:
                r = r[::-1]
                a_inc = -a_inc
                # angle_min/angle_max 관계도 뒤집히지만, 아래는 인덱스 기반 roll이므로 a_min만 보정
                a_min = -float(scan.angle_max)

            # ── 정면(0 rad)을 벡터 중앙에 오도록 원형 정렬(roll) ───────────────────
            #     yaw_off이 있으면 0 대신 yaw_off를 "정면"으로 맞춤
            #     idx_zero: (yaw_off) 각도가 위치한 원시 벡터 인덱스
            idx_zero = int(round((yaw_off - a_min) / a_inc)) % N
            roll = (N // 2) - idx_zero     # 중앙=정면
            r = np.roll(r, roll)

            # ── 균등 섹터 binning(min-pooling) : N -> self.environment_dim ─────────
            nb = int(self.environment_dim)
            if nb <= 0:
                return

            edges = np.linspace(0, N, num=nb + 1, dtype=int)
            # 기본값: 최대 거리(안전)
            state = np.full(nb, rmax, dtype=np.float32)

            # 각 섹터 최소값(가장 가까운 장애물) 선택 → 회피/안전 판단에 유리
            for i in range(nb):
            ...
            # ── (선택) 0~1 정규화가 필요하면 주석 해제 ─────────────────────────────
            # state /= rmax

            # ── 유효 레이 하나도 못 썼으면(이론상 거의 없음) 이전 상태 유지 ───────
            # 여기서는 state가 항상 채워지므로 스킵

            # ── 상태 갱신(스레드 안전) ────────────────────────────────────────────
            with self.environment_state_lock:
                self.environment_state = state
                # 초기 수신 플래그를 쓰고 있다면 표시
                if hasattr(self, "_have_scan"):
                    self._have_scan = True

        except Exception as e:
            self.get_logger().warn(f"[LaserScan] update failed: {e}")
            # 실패 시 이전 상태 유지
            return
    ...
    def update_agent_state(self, odom):
        ...
        with self.agent_state_lock:
		    ...
		    ** reward 관련 코드
		    *****************************************************"""
            now = self.get_clock().now().nanoseconds * 1e-9  # 초 단위
            self._pose_hist.append((now, float(odom_x), float(odom_y)))

            # 1초 창 유지
            t_cut = now - self._hist_window
            while self._pose_hist and self._pose_hist[0][0] < t_cut:
                self._pose_hist.popleft()

            # 창의 맨 처음 위치와 현재 위치의 변위
            if self._pose_hist:
                _, x0, y0 = self._pose_hist[0]
                self._recent_disp = float(np.hypot(odom_x - x0, odom_y - y0))
    ...
    def reset_callback(self, _, response):
		...
		*****************************************************"""
        ...
        self.set_agent_state.pose.position.z = 0.28
    ...
    def check_collision(self, laser_data):
        ...
        collided = (min_laser < self.collision_threshold)
        return bool(collided), bool(collided), min_laser
    ...
    def get_reward(self, target, collision, action, min_laser):
        ...
        # --- 정체(1초 이동거리 부족) 페널티 추가 ---
        r_min = 0.03    # 1초 동안 최소 이동해야 하는 거리 (m)
        stagnation_penalty = -0.3 if self._recent_disp < r_min else 0.0
        return action_reward + obstacle_reward + stagnation_penalty
